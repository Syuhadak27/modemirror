...
<i>Send /stop to Stop Process</i>""")
        ...
        async with session_lock:
            if session_dict['CONFIRM_PHN'].lower() in ['y', 'yes']:
                break
    try:
        pyro_client = Client(f"modemirror-{message.from_user.id}", api_id=api_id, api_hash=api_hash)
...
<i>Send /stop to Stop Process</i>""")
 ...
 <i>Send /stop to Stop Process</i>""")
    ...
    try:
        ...
        await pyro_client.send_message("self", f"⌬ <b><u>Pyrogram Session Generated :</u></b>\n\n<code>{session_string}</code>\n\n<b>Via <a href='https://github.com/weebzone/modemirror'>modemirror</a> [ @modemirror ]</b>", disable_web_page_preview=True)
        await pyro_client.disconnect()
    ...
    try:
        await aioremove(f'modemirror-{message.from_user.id}.session')
        await aioremove(f'modemirror-{message.from_user.id}.session-journal')
    except Exception:
        pass
